...
class _MyHomePageState extends State<MyHomePage> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        ...
        backgroundColor: const Color.fromARGB(238, 255, 136, 1),
      ),
              ...
              child: Container(
                ...
                decoration: const BoxDecoration(
                    ...
                    image: DecorationImage(
                        image: AssetImage('images/logoWatteco.png'))),
              ),
            ),
            ListTile(
    ...
    if (data[0] == 65) {
      ...
      for (int i = 0; i < data.length; i++) {
        if (data[i] == 44) {
          ...
          //if carriage return = new phase
          saveValue = true;
          dataIndex++;
          nbPhasesRead++;
          savePhase = true;
        } else {
          //if data lambda = digit of the value
          if (dataIndex != 0 && (data[i] < 48 || data[i] > 57)) {
            //the digit is not a number => number = 0
            if (data[i] == 45) {
              dataRead.add(45);
            } else {
              dataRead.add(48);
              ...
              if (readingPhase.mode == 'D') {
                readingPhase.deltaVoltage = double.parse(
                    (double.parse(ascii.decode(dataRead)) / 10)
                        .toStringAsFixed(1));
                readingPhase.voltage = double.parse(
                    (readingPhase.deltaVoltage / sqrt(3)).toStringAsFixed(1));
              } else {
                readingPhase.voltage = double.parse(
                    (double.parse(ascii.decode(dataRead)) / 10)
                        .toStringAsFixed(1));
  ...
  bool saveDataIntoClass(
      Phase readingPhase, int nbPhasesRead, BuildContext context) {
